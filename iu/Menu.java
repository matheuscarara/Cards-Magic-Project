...
public class Menu {
	public static void main(String[] args) {
		...
		while (true) {
				...
				case 1:
					...
					try {
						...
						ResultSet retornoBD = consulta
								.executeQuery("SELECT * FROM JOGADOR WHERE LOGIN = " + "'" + login + "';");
						retornoBD.next();
						jogador = new Usuario(retornoBD.getString("login"), retornoBD.getString("senha"));
						logando = false;
						consulta.close();
						retornoBD.close();
						...
						ResultSet retornoBaralho = montabaralho.executeQuery("SELECT CARTAS.NOME, CARTAS.ATAQUE,"
								...
								+ " WHERE CARTAS.ID = BARALHO.IDCARTA AND BARALHO.IDJOGADOR = '" + login + "';");
						while (retornoBaralho.next()) {
							jogador.adicionaCartaNoBaralho(new Carta(retornoBaralho.getString("nome"),
									retornoBaralho.getInt("ataque"), retornoBaralho.getInt("defesa"),
									retornoBaralho.getInt("id"), retornoBaralho.getString("elemento")));
						}
						montabaralho.close();
						retornoBaralho.close();
					} catch (SQLException | ExcecaoCartaNaoExiste | ExcecaoBaralhoCheio e) {
						e.printStackTrace();
					}
					break;
				case 2:
					...
					try {
						...
						criacao.executeUpdate(
								"INSERT INTO JOGADOR (LOGIN,SENHA)" + " VALUES ('" + login + "','" + senha + "');");
						criacao.close();
			...
			while (dentro) {
				...
				case 1:
					if (jogador.getBaralho().getBaralho().isEmpty()) {
						...
						for (int i = 0; i < jogador.getBaralho().getBaralho().size(); i++) {
							...
							texto += " " + jogador.getBaralho().getBaralho().get(i).getElemento();
							texto += "\n";
						}
					}
					JOptionPane.showMessageDialog(null,
							jogador.getBaralho().tamanhoDoBaralho() + " Cartas em seu baralho:\n" + texto);
					break;
				case 2:
					...
					while (alterando) {
						...
						Integer opcao2 = Integer.parseInt(
								JOptionPane.showInputDialog("Digite o Id da carta a ser inserida no baralho.\n"
										+ "Digite 50 para sair. \n" + texto));
						...
						default:
							try {
								Statement consulta = bd.createStatement();
								ResultSet retornoBD = consulta
										.executeQuery("SELECT * FROM CARTAS WHERE ID = '" + opcao2 + "';");
								retornoBD.next();
								...
								adicionado.executeUpdate("INSERT INTO BARALHO (IDCARTA,IDJOGADOR)" + " VALUES ('" + opcao2  + "','" + jogador.getLogin() + "');");
								adicionado.close();
				...
				case 3:
					...
					if (jogador.getBaralho().baralhoEstaCompleto()) {
						...
						JOptionPane.showMessageDialog(null, "Seu baralho esta incompleto.");
						break;
					}
					dentro = false;
					break;
			...
			Integer ladoMoeda = Integer
					.parseInt(JOptionPane.showInputDialog("Digite 0 para selecionar cara ou 1 para coroa"));
			if (ladoMoeda == 0)
			...
			while (noTabuleiro) {
				...
				case 3:
					...
					opcaoAddCartaCampo = Integer
							.parseInt(JOptionPane.showInputDialog("Digite a carta a ser inserida: \n\n" + mao + "\n\n"
									+ "Digite qualquer outro valor para sair."));
				...
				case 4:
					...
					opcaoRemCartaCampo = Integer
							.parseInt(JOptionPane.showInputDialog("Digite a carta a ser removida: \n\n" + campo + "\n\n"
									+ "Digite qualquer outro valor para sair."));
					...
					if (!(opcaoRemCartaCampo >= tabuleiro.getTamanhoDoCampo() || opcaoRemCartaCampo < 0)) {
						try {
							...
							JOptionPane.showMessageDialog(null, "Campo vazio, tente outra vez.");
							break;
						}
					}
					break;
	...
	private static Connection inicializaBancoDeDados(Connection bd) {
		try {
			...
			bd = DriverManager.getConnection("jdbc:postgresql://localhost:5432/cards&magic", "postgres", "10203040");
		} catch (Exception e) {
			e.printStackTrace();
